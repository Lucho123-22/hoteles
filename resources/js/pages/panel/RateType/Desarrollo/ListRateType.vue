<template>
    <Toolbar class="mb-6">
        ...
        <template #end>
            <Button 
                ...
                :loading="rateTypeStore.isLoading"
            />
        </template>
    </Toolbar>
    ...
    >
        ...
        <!-- Columna: Display Name -->
        <Column 
            field="display_name" 
            header="Nombre de Visualización" 
            :sortable="true" 
            style="min-width: 16rem"
        >
            <template #body="{ data }">
                <div class="flex items-center gap-2">
                    <i :class="`pi pi-${data.icon}`"></i>
        ...
        <!-- Columna: Requiere Rango de Tiempo -->
        <Column 
            field="requires_time_range" 
            header="Rango de Tiempo" 
            :sortable="true" 
            style="min-width: 12rem"
        >
            <template #body="{ data }">
                <Tag 
                    :value="data.requires_time_range ? 'Sí' : 'No'"
                    :severity="data.requires_time_range ? 'info' : 'secondary'" 
                />
            </template>
        </Column>
...
const fetchRateTypes = async () => {
    try {
        ...
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: error.message || 'No se pudieron cargar los tipos de tarifa',
            life: 3000
        });
    }
};
        ...
        accept: async () => {
            try {
                ...
                toast.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: error.message || 'No se pudo eliminar el tipo de tarifa',
                    life: 3000
                });
            }
        }
